...
void CreateGUI() {
   ...
   CreateBtn(BTN_RESET, ShortToString(0x2302), PanelX + InpPanelW - 42, y + 2, 18, 18, InpClrInactive, 9);
   ...
   CreateLbl(LBL_PNL, "PnL: 0.00", PanelX + 10, y + 4, InpClrInfoPnLBase, 11, true);
   
   if(InpShowBalance && !IsMinimized) {
      CreateLbl(LBL_BALANCE, "Bal: ...", PanelX + 105, y + 6, InpClrInfoBal, 9, true); 
   }
   
   // Header Row 2: Spread | Pending Count | Pos Summary
   if(!IsMinimized) {
       CreateLbl(LBL_SPREAD, "Sprd: 0", PanelX + 10, y + 24, InpClrInfoSprdBase, 8, false);
       CreateLbl(LBL_PEND, "Pend: 0", PanelX + 75, y + 24, InpClrInfoPendBase, 8, false);
       CreateLbl(LBL_MARG_REQ, "Marg: $0.00", PanelX + 130, y + 24, InpClrInfoMarg, 8, false);
   }
   
   // Position Summary (Net Direction/Volume)
   CreateLbl(LBL_POS_SUMMARY, " ", PanelX + 195, y + 24, InpClrInfoPosBase, 8, false);
   ...
   // --- FOOTER INFORMATION ---
   y += 26; 
   int timerX = PanelX + (InpPanelW / 2);
   CreateLbl(LBL_T_TITLE, "TIME UNTIL CLOSE", timerX - 45, y, InpClrTimerTitle, 7, false);
   CreateLbl(LBL_TIMER, "00:00", timerX - 25, y + 10, InpClrTimer, 12, true);
