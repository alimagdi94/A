...
void OnTimer()
{
   ...
   if(g_RepeatActive && g_RepeatAction != REPEAT_NONE) {
      FireRepeatAction();
      ChartRedraw();
      return; // Skip slow updates during rapid repeat
   }
   ...
   if(id == CHARTEVENT_MOUSE_MOVE && IsPanelVisible) {
      ...
      int mouseState = (int)StringToInteger(sparam); 
       ...
       if(!leftDown && g_RepeatActive) {
        ...
        int panelH = IsMinimized ? 55 : PANEL_H_FULL;
      ...
      if(g_IsDragging) {
         if(leftDown) {
            ...
            if(newX < 0) newX = 0;
            if(newY < 0) newY = 0;
            if(newX + PANEL_W > chartW) newX = chartW - PANEL_W;
...
void CreateGUI() {
   ...
   if(!IsPanelVisible) return;
   int y = PanelY; int panelH = IsMinimized ? 45 : PANEL_H_FULL;
   ...
   ObjectSetInteger(0, BG_PANEL, OBJPROP_XSIZE, PANEL_W);    ObjectSetInteger(0, BG_PANEL, OBJPROP_YSIZE, panelH); 
   ...
   ObjectSetInteger(0, BG_HEADER, OBJPROP_XSIZE, PANEL_W);    ObjectSetInteger(0, BG_HEADER, OBJPROP_YSIZE, 45); 
   ...
   CreateBtn(BTN_MINMAX, IsMinimized ? "+" : "-", PanelX + PANEL_W - 22, y + 2, 18, 18, InpClrInactive, 9);
   ObjectSetString(0, BTN_MINMAX, OBJPROP_TOOLTIP, "Minimize (F) / Hide (R)");
   CreateBtn(BTN_RESET, ShortToString(0x2302), PanelX + PANEL_W - 42, y + 2, 18, 18, InpClrInactive, 9);
   ObjectSetString(0, BTN_RESET, OBJPROP_TOOLTIP, "Reset Position to Default");
   CreateBtn(BTN_DOCK, g_IsDocked ? "L" : "U", PanelX + PANEL_W - 62, y + 2, 18, 18, InpClrInactive, 9);
   ...
   y += 48; // Compact gap before inputs container
   ...
   ObjectSetInteger(0, BG_INPUTS, OBJPROP_XSIZE, PANEL_W - 10);  ObjectSetInteger(0, BG_INPUTS, OBJPROP_YSIZE, 105); 
   ...
   CreateBtn(BTN_TOGGLE_MODE, g_IsManualLotMode ? "Lot [\\]" : "Risk [\\]", PanelX + 10, y, 70, EDIT_H, InpClrGroupBox, 8); 
   ...
   CreateBtn(BTN_RISK_DN, "▼", PanelX + 85, y, 18, EDIT_H, InpClrBtnAdj, 8);
   CreateEdit(EDIT_RISK, g_IsManualLotMode ? DoubleToString(g_LotSize, 2) : DoubleToString(g_RiskUnits, 3), PanelX + 105, y, 40);
   CreateBtn(BTN_RISK_UP, "▲", PanelX + 147, y, 18, EDIT_H, InpClrBtnAdj, 8);
   CreateLbl(PREFIX+"L1", "Lot", PanelX + 170, y + 2, InpClrText, 8, true);
   CreateEdit(EDIT_LOT, DoubleToString(g_LotSize, 2), PanelX + 190, y, 40); 
   ...
   CreateBtn(BTN_SL_DN, "▼", PanelX + 52, y, 18, EDIT_H, InpClrBtnAdj, 8); 
   CreateEdit(EDIT_SL, IntegerToString(g_SL_Points), PanelX + 72, y, 40); 
   ...
   CreateBtn(BTN_TP_DN, "▼", PanelX + 52, y, 18, EDIT_H, InpClrBtnAdj, 8); 
   CreateEdit(EDIT_TP, IntegerToString(g_TP_Points), PanelX + 72, y, 40); 
   CreateBtn(BTN_TP_UP, "▲", PanelX + 114, y, 18, EDIT_H, InpClrBtnAdj, 8); 
   CreateBtn(BTN_USE_TP, EnableTP ? "✔" : "✖", PanelX + 134, y, 18, EDIT_H, EnableTP ? InpClrActive : InpClrInactive, 8);
   CreateBtn(BTN_ASGN_TP, "ASGN(H)", PanelX + 156, y, 55, EDIT_H, InpClrAsgnTP, 8);
   CreateBtn(BTN_BE, "B/E(1)", PanelX + 215, y, 45, EDIT_H, InpClrAsgnSL, 8);
   ...
   if(g_ExecMode == MODE_PENDING) {
      ...
      CreateBtn(BTN_MODE, modeText, PanelX + 10, y, PANEL_W - 20, 22, InpClrHeader, 8);
      if(ObjectFind(0, BTN_ENTRY_UP) >= 0) ObjectDelete(0, BTN_ENTRY_UP);
      if(ObjectFind(0, BTN_ENTRY_DN) >= 0) ObjectDelete(0, BTN_ENTRY_DN);
   }

   y += 26; 
   int btnW = (PANEL_W - 25) / 2; 
   ...
   y += 28;
   int btnW4 = (PANEL_W - 25) / 4; 
   CreateBtn(BTN_C_BUY, "CL BUY(Z)", PanelX + 10, y, btnW4, 20, InpClrClose, 8); 
   CreateBtn(BTN_C_SELL, "CL SEL(X)", PanelX + 15 + btnW4, y, btnW4, 20, InpClrClose, 8);
   CreateBtn(BTN_C_WIN, "CL WIN(C)", PanelX + 20 + (btnW4 * 2), y, btnW4, 20, InpClrClose, 8); 
   CreateBtn(BTN_C_LOSS, "CL LOS(D)", PanelX + 25 + (btnW4 * 3), y, btnW4, 20, InpClrClose, 8);
   
   y += 24;
   CreateBtn(BTN_C_PEND, "CAN PEND(A)", PanelX + 10, y, PANEL_W - 20, 20, InpClrClose, 8);
